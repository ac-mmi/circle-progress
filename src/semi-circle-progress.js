 ...
 */
class SemiCircleProgress extends CustomElement {
	...
	/**
	 * Construct the new SemiCircleProgress instance
	 * @constructs
	...
	constructor(opts = {}) {
		...
		Object.defineProperties(
			this,
			Object.keys(SemiCircleProgress.props).reduce(
				(descriptors, prop) => {
					descriptors[prop] = {
						get() {
		...
		opts = {...SemiCircleProgress.defaults, ...opts};
		...
		this._initText();
		Object.keys(SemiCircleProgress.props)
	 ...
	 * @return {SemiCircleProgress}       The SemiCircleProgress instance
	 */
	attr(attrs) {
	...
	_set(key, val) {
		...
		if(val === undefined) throw new TypeError(`Failed to set the ${key} property on SemiCircleProgress: The provided value is non-finite.`);
		...
		switch(key) {
			...
			case 'textFormat':
				if(typeof val !== 'function' && !['valueOnCircle', 'horizontal', 'vertical', 'percent', 'value', 'none'].includes(val)) {
					throw new Error(`Failed to set the "textFormat" property on SemiCircleProgress: the provided value "${val}" is not a legal textFormat identifier.`);
				}
				break;
			case 'animation':
				if(typeof val !== 'string' && typeof val !== 'function') {
					throw new TypeError(`Failed to set "animation" property on SemiCircleProgress: the value must be either string or function, ${typeof val} passed.`);
				}
				if(typeof val === 'string' && val !== 'none' && !easings[val]) {
					throw new Error(`Failed to set "animation" on SemiCircleProgress: the provided value ${val} is not a legal easing function name.`);
				}
				break;
		}
		return val;
	...
	_valueToAngle(value = this.value) {
		return Math.min(
			...
			Math.max(
				0,
				(value - this.min) / (this.max - this.min) * 180
			)
		);
	}
	 ...
	 *                   as returned by {@link SemiCircleProgress.getRadius}, where text should be.
	...
	/**
	 * Generate text representation of the values based on {@link SemiCircleProgress#textFormat}
	 */
	_initText() {
	 ...
	 */
	updateGraph() {
		const startAngle = this.startAngle - 180;
...
customElements.define('semi-circle-progress', SemiCircleProgress);
